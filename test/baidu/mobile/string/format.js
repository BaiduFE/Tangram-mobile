module("baidu.mobile.string.format测试");

test("默认分隔符Json数据格式化字符串", function(){
	var sPattern = "id=#{id}?+&action=#{action}&||type=#{type}.*result=#{result}";
	
	var sRet;
	sRet = baidu.mobile.string.format(sPattern, {id:1, action:"post", type:"node", result:5.1});
	equals(sRet, "id=1?+&action=post&||type=node.*result=5.1");
	
	var sAction = new String("\x25\u31f2");
		sRet = baidu.mobile.string.format(sPattern, {id:0, action:sAction, type:"&nsbp;$1$2", result:"\\\%@'64K*[^]"});
	...
	sRet = baidu.mobile.string.format(sPattern, {b:"hello字符"});
...
test("默认分隔符多个参数格式化字符串", function(){
	...
	sRet = baidu.mobile.string.format(sPattern, 12, "B$1", "code");
	...
	sRet = baidu.mobile.string.format(sPattern, "A", sStr, "C");
	equals(sRet, "a:A|b:showbug|c:C");
	
	sRet = baidu.mobile.string.format(sPattern, sStr, "B", "C");
	equals(sRet, "a:showbug|b:B|c:C");
	
	sPattern = "#{0}|#{2}|#{4}";
	sRet = baidu.mobile.string.format(sPattern, "a", "b", "c", "d", "e");
	equals(sRet, "a|c|e");
}); // 2
...
test("特殊case", function(){
	...
	//第一个0
	sPattern = "#{0},#{1}";
	sRet = baidu.mobile.string.format(sPattern, 0, "abc");
	equals(sRet, "0,abc");
	
	//第一个""
	sPattern = "#{0},#{1}";
	sRet = baidu.mobile.string.format(sPattern, "", "abc");
	equals(sRet, ",abc");
	
	//funciton
	sPattern = "#{0}";
	sRet = baidu.mobile.string.format(sPattern, function(){return 'abc';});
	equals(sRet, "abc");
});  // 4
...
test("异常case1", function(){
	...
	sRet = baidu.mobile.string.format(sPattern, {a:"A"});
	equals(sRet, "a:A,b=");
});  // 5
...
test("异常case2", function(){
	...
	var sRet = baidu.mobile.string.format(sPattern, {a:"A",c:"C",0:"B",1:"D"});
...
test("异常case3", function(){
	//顺序颠倒
	sPattern = "#{1}|#{0}";
	sRet = baidu.mobile.string.format(sPattern, "A", "B");
	equals(sRet, "B|A");
});  // 7
...
test("异常case5", function(){
	//#{}括号内包含特殊字符{
	sPattern = "#{ab{cd}";
	sRet = baidu.mobile.string.format(sPattern, {"ab{cd":"OK"});
	equals(sRet, "OK");
});  // 9
...
test("异常case6", function(){
	//特殊对象 RegExp
	sPattern = "#{0}";
	sRet = baidu.mobile.string.format(sPattern, /a/);
	equals(sRet, "/a/");
});  // 10

test("异常case7", function(){
	sPattern = "#{0}";
	sRet = baidu.mobile.string.format(sPattern, null);
	equals(sRet, "null");
});  // 11

//test("异常case8", function(){
//	sPattern = "#{0}";
//	sRet = baidu.mobile.string.format(sPattern, void(0));
//	equals(sRet, "0");
//});  // 12
...
test("异常case10", function(){
	sPattern = "#{0}#{1}";
	sRet = baidu.mobile.string.format(sPattern, [,false]);
	equals(sRet, "false");
});  // 14

test("异常case11", function(){
	sPattern = "#{0} #{1} #{2} #{3} #{4} #{5}";
	sRet = baidu.mobile.string.format(sPattern, [,false, 100, function(){return 123}, [1,2], {a:"a"}]);
	equals(sRet, " false 100 123 1,2 [object Object]");
});  // 15



//describe('baidu.mobile.string.format测试', {
...
//
//		var sAction = new String("\x25\u31f2");
//		sRet = baidu.mobile.string.format(sPattern, {id:0, action:sAction, type:"&nbsp;$1$2",result:"\\\%@'64K*[^]"});
...
//		sRet = baidu.mobile.string.format(sPattern, {a:"A",c:"C",0:"B",1:"D"});
